...
import { EXTENSION_ID } from '../src/index.js';
export async function initViewer(container, getToken, onTokenError) {
    ...
    await new Promise(resolve => Autodesk.Viewing.Initializer({
...
function loadDocument(urn) {
    return new Promise((resolve, reject) => {
        Autodesk.Viewing.Document.load(urn.startsWith('urn:') ? urn : `urn:${urn}`, resolve, (code, message) => reject(new Error(`Could not load document ${urn}: ${code} ${message}`)));
    });
}
let latestRequest = 0;
/**
 ...
 */
export async function loadDesign(viewer, urn, name) {

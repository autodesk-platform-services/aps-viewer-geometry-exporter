...
function createReaderTarget() {
    const Vector3 = globalThis.Autodesk?.Viewing?.Math?.Vector3;
 ...
 */
export function convertGeometry(geom, mode, diagnostics, context) {
 ...
 */
function readNormals(geom, vertexCount, target, diagnostics, context) {
 ...
 */
export function decodeSphericalNormals(packed) {
...
function readVec3(reader, count, target) {
 ...
 */
function cleanIndices(indices, vertexCount, primitiveSize, diagnostics, context, raw) {
...
function readOptionalAttribute(geom, name, allowedItemSizes, vertexCount, diagnostics, context) {
...
export function describeAttribute(attr) {
    if (!attr) {
...
const INTEGER_NORMALIZERS = new Map([
 ...
 */
export function decodeAttribute(attr, allowedItemSizes, vertexCount) {
    if (!attr) {
    ...
    if (!interleaved) {
        ...
        const bytesPerItem = attr.bytesPerItem ?? 4;
        if (bytesPerItem === 4) {
...
//# sourceMappingURL=convertGeometry.js.map
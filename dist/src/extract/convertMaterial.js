...
export function classifyMaterialSource(material) {
    const scene = globalThis.Autodesk?.Viewing?.Scene;
    ...
    if (type === 'PrismMaterial' || material?.isPrismMaterial === true) {
 ...
 */
function accessor(source, legacy, getter, property) {
...
const legacyVertexColors = (m) => () => typeof m.vertexColors === 'number' ? m.vertexColors !== 0 : m.vertexColors;
/** Rough Blinn-Phong exponent → GGX roughness conversion. */
export function specularPowerToRoughness(power) {
...
const KNOWN_WRAPPING = new Set([THREE.RepeatWrapping, THREE.ClampToEdgeWrapping, THREE.MirroredRepeatWrapping]);
/**
...
export class MaterialConverter {
    ...
    convertUncached(material, context) {
        ...
        const get = (getter, property) => accessor(m, legacy, getter, property);
     ...
     */
    convertPrism(m, ctx) {
        const color = (property) => this.readColor(() => m[property], property, ctx, m, THREE.LinearSRGBColorSpace);
        const scalar = (property, fallback) => this.readScalar(() => m[property], property, fallback, ctx, m);
        const unit = (property, fallback) => Math.min(1, scalar(property, fallback));
        const unsupported = [];
        const params = {
        ...
        switch (m.prismType) {
            case 'PrismOpaque':
                ...
                if (m.opaque_luminance > 0)
                    unsupported.push('emission (opaque_luminance)');
                break;
            case 'PrismMetal':
    ...
    readColor(get, what, ctx, raw, colorSpace = THREE.SRGBColorSpace) {
        ...
        return out.setRGB(clamp(value.r), clamp(value.g), clamp(value.b), colorSpace);
    }
    readScalar(get, what, fallback, ctx, raw) {
        const value = this.diagnostics.guard(ctx, `Material.get${capitalize(what)}`, get, undefined);
    ...
    readUnitScalar(get, what, fallback, ctx, raw) {
    ...
    readBoolean(get, what, fallback, ctx, raw) {
        const value = this.diagnostics.guard(ctx, `Material.get${capitalize(what)}`, get, undefined);
    ...
    readSide(get, ctx, raw) {
        const value = this.diagnostics.guard(ctx, 'Material.getSide', get, undefined);
        const Side = globalThis.Autodesk?.Viewing?.Scene?.Side;
        if (Side && value === Side.Front)
            return THREE.FrontSide;
        if (Side && value === Side.Back)
            return THREE.BackSide;
        if (Side && value === Side.Double)
...
function imageToCanvas(image, d, ctx, raw) {
    ...
    try {
        ...
        const isDrawable = (typeof HTMLImageElement !== 'undefined' && image instanceof HTMLImageElement) ||
...
function transmissionToOpacity(transmission) {
...
//# sourceMappingURL=convertMaterial.js.map